...
def mining_pubs(tipo):
    ...
    if tipo == "DP":
        ...
        year = re.findall("DP\s{2}-\s(\d{4})", texto) ##para extraer el año de publicación de los articulos 
    ...
    ## Data frame con el PMID y año de publicación 
        ...
        for i in range(len(AD)): 
            ...
            if not pais == []: ## para eliminar listas vacias que no contengan información de los paises 
                if not len(pais) >= 2:   
                    if re.findall("^[A-Z]", pais[0]): ## para eliminar secciones que no empiezan con mayusculas
